...
| `user_id` | `INT` | NOT NULL, INDEX, FK → `users.id` |
...
> **归属变更**：后端已从 Flask + MySQL/Alembic 切换到 Spring Boot + PostgreSQL/Flyway。schema 现在由 **Spring 端 Flyway 拥有**（不再只是 baseline，参考来源仍是 `flask-app/migrations/versions/*`）。

- `V1__baseline.sql`：**完整的 PostgreSQL 初始化 DDL**，包含上述 6 张表（`station` / `availability` / `users` / `weather_forecast` / `sessions` / `message_store`）以及：
  - `availability.number`、`availability."timestamp"`、`sessions.user_id` 上的索引；
  - `users` 的三个唯一索引（`username` / `email` / `activation_token`）和 `weather_forecast.forecast_time` 唯一索引；
  - `set_updated_at_timestamp()` PL/pgSQL 函数 + `users` 的 `BEFORE UPDATE` 触发器，用来填充 `updated_at`（因为 JPA 实体把这列标成 `insertable=false, updatable=false`，没有触发器会一直为 NULL）。
- `Float` 字段在 DDL 中用 `REAL`、`LocalDateTime` 用 `TIMESTAMP`（无时区）、自增主键统一用 `GENERATED BY DEFAULT AS IDENTITY`。
- 配置：
  ```yaml
  spring:
    flyway:
  ...
  ```
  > `baseline-on-migrate` 保留以兼容"对接已有 Postgres 库"的情况；全新库会直接执行 V1。
- 后续 schema 变更走 `V2__xxx.sql`、`V3__xxx.sql` …，**禁止**回头修改已应用的版本文件。
- **不要**让 Hibernate 自动 DDL：`spring.jpa.hibernate.ddl-auto: validate`。任何字段增删/类型变更必须先写新的 `V*.sql`，再同步修改实体。
